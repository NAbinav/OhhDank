...
.then(res=>{
  ...
  return (
    <tr>
        ...
        <td><div className='flex'><IndianRupee/>{product.price}</div></td>
        <td className='py-2'><Image className='rounded-lg' src={product.imgSrc} height={250} width={250} alt={`${product.name} Image`}/></td>
        <td><div className='text-2xl flex items-center gap-2 text-gray-600'><Edit className='cursor-pointer hover:text-black' onClick={onEdit}/><Trash2 className='cursor-pointer hover:text-black' onClick={onDelete}/></div></td>
    </tr>
  )
}
